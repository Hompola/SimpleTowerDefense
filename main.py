...
def CreateTurret(mouse_pos, turretType):
  ...
  if world.tilemap[row][col] == 0:
    ...
    if space_is_free == True:
      newTurret = Turret(turret_sheet, row, col, turretType)
...
def LoadTurretData(turretType):
  ...
  turret_sheet_loc = turret_data.TURRET_DATA.get(turretType).get("animSheet")
  cursorTurretImage_loc = turret_data.TURRET_DATA.get(turretType).get("cursorImage")
  turret_sheet = pg.image.load(turret_sheet_loc).convert_alpha()
  ...
  return turret_sheet, cursorTurretImage, turretBuyCost

def DrawTurretButtons():
  pass
...
while run:
  ...
  if placing_turrets == True:
    ...
    DrawText(str(wizardCost), "small", "grey0", c.SCREEN_WIDTH + 90 + 12, 290)
    ...
    if event.type == pg.MOUSEBUTTONDOWN and event.button == 1:
      ...
      if mousePos[0] < c.SCREEN_WIDTH and mousePos[1] < c.SCREEN_HEIGHT:
        if placing_turrets == True:
          ...
          selected_turret = select_turret(mousePos)
